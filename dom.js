...
let h2_head = createDivHeader(header, "h2", "Checkout form");

let p_head = createDivHeader(
  header,
  "p",
...
form_bill.appendChild(row_1)
  ...
  row_1.appendChild(div1_row1)
    ...
    //inicio contenido div 1 del row 1

    let label_div1 = constructorLabels('first name', 'First name')
    ...
    let feedback_div1 = constructorDivs('div', 'invalid-feedback', 'Valid last name is required.')
    div1_row1.appendChild(feedback_div1)
    // fin

    // inicio contenido div 2 row 1
    let div2_row1 = constructorDivs('div', 'col-md-6 mb-3')
    row_1.appendChild(div2_row1);

    let label_div2 = constructorLabels('first name', 'First name')
    div2_row1.appendChild(label_div2)

    let input_div2 = constructorInputs("form-control", "text", "", true, "firstname" );
    div2_row1.appendChild(input_div2);

    let feedback_div2 = constructorDivs('div', 'invalid-feedback', 'Valid last name is required.')
    ...
    div_inputgroup_username.appendChild(prepend_username);
        ...
        let invalid_username = constructorDivs('div', 'invalid-feedback', 'Your username is required.')
        div_inputgroup_username.appendChild(invalid_username)
        // username

        //inicio email(opcional)
        ...
        div_email.appendChild(label_email)
            ...
            div_address.appendChild(label_address)
                ...
                let input_address = constructorInputs('form-control', 'text', '1234 main st', true, 'address')
                div_address.appendChild(input_address)
    
                let feedback_address = constructorDivs('div', 'invalid-feedback', 'Please enter a valid email address for shipping updates.')
  ...
  let row_country = constructorDivs('row', 'row')
  ...
  //inicio div country   
    ...
    let select_state = constructorSelects('custom-select d-block w-100', 'state', true, ['Choose...', 'California'])
...
div_billing.appendChild(div_pago);
  ...
  div_pago.appendChild(div_label_pago);
    ...
    let input_pago = constructorInputs('form-control', 'text', '', true, 'cc-number')
    div_label_pago.appendChild(input_pago)

    let small_pago = constructorDivs("small", "text-muted");
    small_pago.appendChild(document.createTextNode("Full name as displayed on card"));
    div_label_pago.appendChild(small_pago);

    let feedback_pago = constructorDivs('div', 'invalid-feedback', 'Name on card is required')
  ...
  div_pago.appendChild(div_label_credit);
    ...
    let input_credit = constructorInputs('form-control', 'text', '', true, 'cc-number')
    ...
    let feedback_credit = constructorDivs('div', 'invalid-feedback', 'Credit card number is required')
  ...
  div_expiracion.appendChild(div_label_expiracion);
    ...
    let label_expiracion = constructorLabels('label', 'Expiration', 'cc-expiration' )
    div_label_expiracion.appendChild(label_expiracion);

    let input_expiracion = constructorInputs('form-control', 'text', '', true, 'cc-number')
    div_label_expiracion.appendChild(input_expiracion)

    let feedback_expiracion = constructorDivs('div', 'invalid-feedback', 'expiration date required')
    ...
    let feedback_cvv = constructorDivs('div', 'invalid-feedback', 'Security code required')
  ...
  let boton_con = constructorBotones('submit', 'btn btn-primary btn-lg btn-block', 'Continue to checkout')
  div_billing.appendChild(boton_con)
